...
// 修改AWEFeedIphoneAutoPlayManager的钩子，直接控制原生自动连播
...
- (BOOL)isAutoPlayOpen {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYisEnableAutoPlay"]) {
        // 启用原生自动连播
        [self _setNativeAutoPlayEnabled:YES];
        return YES;
    }
    // 其他情况使用原生逻辑
    return %orig;
...
// 新增方法用于设置原生自动连播状态
%new
- (void)_setNativeAutoPlayEnabled:(BOOL)enabled {
    // 假设存在名为_setAutoPlayEnabled的内部方法
    SEL selector = NSSelectorFromString(@"setAutoPlayEnabled:");
    if ([self respondsToSelector:selector]) {
        ((void (*)(id, SEL, BOOL))objc_msgSend)(self, selector, enabled);
    } else {
        // 备选方案：直接修改实例变量（需确认变量名）
        objc_setAssociatedObject(self, @selector(autoPlayEnabled), @(enabled), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}

%end

// 修改AWEFeedModuleService的钩子，返回DYYY的开关状态
%hook AWEFeedModuleService

- (BOOL)getFeedIphoneAutoPlayState {
    // 直接返回DYYY的开关状态
    return [[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYisEnableAutoPlay"];
}
...
- (void)setBool:(BOOL)value forKey:(NSString *)defaultName {
    if ([defaultName isEqualToString:@"DYYYisEnableAutoPlay"]) {
        // 当DYYY开关变化时，更新原生状态
        AWEFeedIphoneAutoPlayManager *manager = [%c(AWEFeedIphoneAutoPlayManager) sharedInstance];
        [manager _setNativeAutoPlayEnabled:value];
    }
    %orig;
}
